...
export async function GET(request: Request) {
  try {
    ...
    const rows = await prisma.municipalityImmigrationStat.findMany({
      where: {
        ...(year !== undefined ? { year } : {}),
    ...
    console.error("[api/immigrering]", error);
    return NextResponse.json(
      { error: "Kunne ikke hente innvandringsdata fra databasen." },
      { status: 500 },
    );
  }
}
